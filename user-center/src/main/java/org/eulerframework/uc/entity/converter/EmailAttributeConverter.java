 ...
 * email address as an encrypted envelope string. Used for the
 * {@code email} column of {@code t_user_identity_email} so the raw PII
 * never lands on disk in clear text.
 */
@Converter
public class EmailAttributeConverter extends AbstractEncryptedAttributeConverter<String> {

    public EmailAttributeConverter(DataCipher dataCipher) {
        super(dataCipher);
    }
