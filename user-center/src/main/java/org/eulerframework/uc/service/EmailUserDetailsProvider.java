...
@Service
@Order(1)
public class EmailUserDetailsProvider implements LocalEulerUserDetailsProvider {
    @Resource
    private UserService userService;

    @Override
    public EulerUserDetails provide(String principal) {
        return UserDetailsUtils.toEulerUserDetails(this.userService.loadUserByEmail(principal));
    }
}
