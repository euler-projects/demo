...
| 字段 | 类型 | 含义 |
|---|---|---|
| `identity_id` | string | **登录身份 ID**<br>由服务端生成的 UUID, 可唯一定位到一个用户的一个具体登录身份 |
| `identity_type` | string | **登录身份类型**<br>取值不限, 可任意扩展; 同一账号下可重复, 例如绑定两个手机号, 即存在两条 `identity_type = "phone"` 的记录. |
| `identifier` | string | **登录身份的唯一标识**<br>不同 `identity_type` 各自定义其含义(如 `wechat` 取值为 `openid`原值、`phone` / `email` 取值为原始手机号 / 邮箱的哈希值). |
...
| 字段 | 类型 | 含义 |
|---|---|---|
| `identifier` | string | **登录身份的唯一标识**<br>对 `wechat` 而言取值与 `openid` 相同 |
