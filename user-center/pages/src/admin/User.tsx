...
import {App as AntdApp, Button, Modal, Form, Input, Select, Checkbox, Spin} from 'antd';
...
import type {OverflowTagItem, RowAction} from './_shared/tableLayout';
import {apiPost, apiPut, apiDelete, apiGet, extractApiError} from './_shared/api';
...
/** One row of the user list (`/admin/api/users`). */
interface UserRow {
    userId: string;
    username: string;
...
/** Values collected by the create / reset / toggle user form. */
...
/** Per-user async phone lookup state (identity fetch keyed by userId). */
...
const listUsers = async ({offset, limit}: {offset: number; limit: number}): Promise<UserRow[]> => {
...
const User = () => {
    ...
    const authorityLabel = (authority: Authority) => {
    ...
    // --- Async phone loading ---
    const [phoneMap, setPhoneMap] = useState<Record<string, PhoneEntry>>({});

    const loadPhonesForUsers = useCallback((users: UserRow[]) => {
        if (!users || users.length === 0) return;
        const updates: Record<string, PhoneEntry> = {};
        users.forEach(u => {
        ...
        users.forEach(u => {
            apiGet<Array<{phone?: string}>>(`/admin/api/users/${encodeURIComponent(u.userId)}/identities?identityType=phone`)
                .then(identities => {
                    const phones = (identities || []).map(i => i.phone).filter((p): p is string => Boolean(p));
                    setPhoneMap(prev => ({
                        ...prev,
    ...
    const renderPhone = (_: unknown, record: UserRow) => {
    ...
    const columns = useMemo<TableProps<UserRow>['columns']>(() => [
        {
            // Two-line identity cell: the captioned "User ID" (i18n key
            // `user.column.userId`) is the `username` — username IS the
            // user-facing identifier in this system — while the opaque
            // `userId` sits underneath as the copyable secondary line.
        ...
        },
        {
            ...
            render: (authorities) => {
                const items = ((authorities ?? []) as Authority[]).map((authority) => ({
    ...
    const showResetPasswordModal = (user: UserRow) => {
    ...
    const showConfirmModal = (user: UserRow, operateType: string) => {
    ...
    const toUserRequestObject = (user: UserFormValues) => ({
        userId: user.userId,
    ...
    const onFinish = (userData: UserFormValues) => {
    ...
    const [formType, setFormType] = useState<string>(OP_TYPE_CREATE);
    ...
    const handleTableChange: NonNullable<TableProps<UserRow>['onChange']> = (pagination, filters, sorter) => {
    ...
    const UserForm = ({operationType}: {operationType: string}) => {
