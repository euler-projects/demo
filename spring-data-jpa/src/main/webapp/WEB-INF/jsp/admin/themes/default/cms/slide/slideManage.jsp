...
<div id="e-dg-zone" data-options="region:'north',split:true" style="background:#eee;">
    <div id="e-dg-tb">
        ...
        <div class="filter">
            ...
            <select id="e-dg-tb-filter-usertype" class="easyui-combobox e-dg-tb-filter" data-filter-name="type" style="width:120px;" 
                data-options="
                    url:'${__ADMIN_PATH}/ajax/cms/slide/findAllSlideTypes',
                    method:'get',
                    panelHeight:'auto',
    ...
    <table id="e-dg" class="easyui-datagrid"
        data-options="
            fit:true,
            url:'${__ADMIN_PATH}/ajax/cms/slide/findSlideByPage',
            toolbar:'#e-dg-tb',
            fitColumns:true,
            rownumbers:true,
            remoteSort:false,
...
<div id="e-ds-zone" data-options="region:'center'">
    ...
    <%-- --%>
    <div id="e-ds-dlg-add-slide" class="easyui-dialog e-ds-dlg" title="${e:i18n('_ADMIN_DASHBOARD_DLG_ADD')}"
        data-options="
            closed:true,
            closable:true,
            draggable:false,
    ...
    <div data-dlg="#e-ds-dlg-add-slide" class="e-ds-dlg-btns">
        <a href="javascript:void(0)" class="easyui-linkbutton e-ds-dlg-btn" onclick="addSlide()"><i class="fa fa-save"></i>${e:i18n('_ADMIN_DASHBOARD_DLG_SAVE')}</a>
        <a href="javascript:void(0)" class="easyui-linkbutton e-ds-dlg-btn" onclick="$('#e-ds-dlg-add-slide').dialog('close')"><i class="fa fa-close"></i>${e:i18n('_ADMIN_DASHBOARD_DLG_CANCEL')}</a>
    </div>
    <%-- --%>
</div>
...
function onAdd() {
    ...
    $('#e-ds-dlg-add-slide').dialog('open').dialog('setTitle', "${e:i18n('_ADMIN_DASHBOARD_DLG_ADD')}");
}
...
function addSlide() {
    ...
    $.ajax({
        url:'${__ADMIN_PATH}/ajax/cms/slide/saveSlide',
        type:'POST',
        async:true,
        data: data,
...
function onClickRow(rowIndex, rowData) {
    ...
    var slideId = getDashboardDataId();
    if(rowData.id == slideId) {
        console.log(rowData.id + '的编辑区已打开,忽略此次点击');
        return;        
    }
    $('#e-ds-dlg-add-slide').dialog('close');
    $('#e-ds-dlg-add-slide').dialog('open').dialog('setTitle', "${e:i18n('_ADMIN_DASHBOARD_DLG_EDIT')} - " + rowData.title);
    ...
    if(row == null || row.length < 1){
        ...
        euler.msg.confirm("确定删除选定图片?", function(r) {
            if(r) {
                ...
                $.ajax({
                    url:'${__ADMIN_PATH}/ajax/cms/slide/deleteSlides',
                    type:'POST',
                    async:true,
                    contentType: false,
                    cache: false,
...
function onSort() {
    ...
    $.ajax({
        url:'${__ADMIN_PATH}/ajax/cms/slide/sortSlides',
        type:'POST',
        async:true,
        contentType: false,
        cache: false,
