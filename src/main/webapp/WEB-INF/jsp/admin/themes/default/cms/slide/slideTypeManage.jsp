...
<div id="e-dg-zone" data-options="region:'north',split:true" style="background:#eee;">
    ...
    <table id="e-dg" class="easyui-datagrid"
        data-options="
            fit:true,
            url:'${__ADMIN_PATH}/ajax/cms/slide/findSlideTypeByPage',
            toolbar:'#e-dg-tb',
            fitColumns:true,
            rownumbers:true,
            remoteSort:false,
...
<div id="e-ds-zone" data-options="region:'center'">
    ...
    <%-- --%>
    <div id="e-ds-dlg-add-slide-type" class="easyui-dialog e-ds-dlg" title="${e:i18n('_ADMIN_DASHBOARD_DLG_ADD')}"
        data-options="
            closed:true,
            closable:true,
            draggable:false,
    ...
    <div data-dlg="#e-ds-dlg-add-slide-type" class="e-ds-dlg-btns">
        <a href="javascript:void(0)" class="easyui-linkbutton e-ds-dlg-btn" onclick="addSlideType()"><i class="fa fa-save"></i>${e:i18n('_ADMIN_DASHBOARD_DLG_SAVE')}</a>
        <a href="javascript:void(0)" class="easyui-linkbutton e-ds-dlg-btn" onclick="$('#e-ds-dlg-add-slide-type').dialog('close')"><i class="fa fa-close"></i>${e:i18n('_ADMIN_DASHBOARD_DLG_CANCEL')}</a>
    </div>
    <%-- --%>
</div>
...
function onAdd() {
    ...
    $('#e-ds-dlg-add-slide-type').dialog('open').dialog('setTitle', "${e:i18n('_ADMIN_DASHBOARD_DLG_ADD')}");
}
...
function addSlideType() {
    ...
    $.ajax({
        url:'${__ADMIN_PATH}/ajax/cms/slide/saveSlideType',
        type:'POST',
        async:true,
        data: data,
...
function onClickRow(rowIndex, rowData) {
    ...
    $('#e-ds-dlg-add-slide-type').dialog('open').dialog('setTitle', "${e:i18n('_ADMIN_DASHBOARD_DLG_EDIT')} - " + rowData.name);
    ...
    if(row == null || row.length < 1){
        ...
        euler.msg.confirm("确定删除选定图片类型?", function(r) {
            if(r) {
                ...
                $.ajax({
                    url:'${__ADMIN_PATH}/ajax/cms/slide/deleteSlideTypes',
                    type:'POST',
                    async:true,
                    contentType: false,
                    cache: false,
